...
@timeit
@aws_handle_regions
def get_eni_data(boto3_session: boto3.session.Session, region: str) -> List[Dict]:
...
def _make_redshift_cluster_arn(region: str, aws_account_id: str, eni_identifier: str) -> str:
    """Cluster ARN format: https://docs.aws.amazon.com/redshift/latest/mgmt/redshift-iam-access-control-overview.html"""
    return f'arn:aws:ec2:{region}:{aws_account_id}:network-interface/{eni_identifier}'


def transform_eni_data(enis: List[Dict], region: str, current_aws_account_id: str) -> None:
    for eni in enis:
        eni['arn'] = _make_redshift_cluster_arn(region, current_aws_account_id, eni["NetworkInterfaceId"])
        eni['ClusterCreateTime'] = str(eni['ClusterCreateTime']) if 'ClusterCreateTime' in eni else None
...
def sync_enis(
    ...
    data = get_eni_data(boto3_session, region)
    transform_eni_data(data, region, current_aws_account_id)
    load_eni_data(neo4j_session, data, region, current_aws_account_id, aws_update_tag)
