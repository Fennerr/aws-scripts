...
from lib.jobs.util.common import parse_and_validate_aws_requested_syncs
from lib.cartography.stats import get_stats_client
from lib.cartography.util import merge_module_sync_metadata
from lib.cartography.util import run_analysis_job
...
def _autodiscover_account_regions(boto3_session: boto3.session.Session, account_id: str) -> List[str]:
    ...
    try:
        ...
        raise
    #return regions
    regions = ["eu-west-1","eu-central-1","af-south-1","us-east-1"]
    return  regions
...
@timeit
def start_aws_ingestion(neo4j_session: neo4j.Session, config: Config, aws_profile_name='Default') -> None:
    common_job_parameters = {
